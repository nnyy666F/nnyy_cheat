...
namespace Touch {
    ...
    std::vector<Vector2> GetAllFingerPositions() {
        ...
        for (auto &device : devices) {
            for (auto &finger : device.Finger) {
                if (finger.isDown) {
                    Vector2 screenPos = Touch2Screen(finger.pos);
                    if (finger.action == TouchAction::DOWN || finger.action == TouchAction::MOVE) {
                        screenPos.x = -screenPos.x; // X轴取负
                        screenPos.y = -screenPos.y; // Y轴取负
                    }
    ...
    void Down(float x, float y) {
        ...
        touch.isDown = true;
        touch.action = TouchAction::DOWN; // 标记为按下操作
        Upload();
        lock.unlock();
    }
    void Move(touchObj *touch, float x, float y) {
        lock.lock();
        touch->pos = Vector2(x, y) * touch_scale;
        touch->action = TouchAction::MOVE; // 标记为移动操作
        Upload();
        lock.unlock();
    }
    ...
    void Up() {
        ...
        touchObj &touch = devices[0].Finger[9];
        touch.isDown = false;
        touch.action = TouchAction::NONE; // 重置为无操作
        Upload();
        lock.unlock();
    }
