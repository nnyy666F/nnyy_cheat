...
void game_wwqy::Touch()
{
    ...
    while(true){
        ...
        for (const auto& touch : touches) {
            ...
            if (this->face == 0) {
                ...
                if (touch.x > (ctrl.fwy - ctrl.fwdx) && touch.x < (ctrl.fwy + ctrl.fwdx) && 
                    (屏幕宽 - touch.y) > (ctrl.fwx - ctrl.fwdx) && (屏幕宽 - touch.y) < (ctrl.fwx + ctrl.fwdx)) {
                    triggerTarget = true;
                }
            }
            if (triggerTarget) {
        ...
        for (int i = 0; i < eneity_count; i++) {
            if (!isInit[i] || Bone_Pos[i].bone_count <= 0 || isMiss[i]) continue;
            ...
            if (predictScreen.x < 1.0f || predictScreen.x > 屏幕宽-1.0f || 
                predictScreen.y < 1.0f || predictScreen.y > 屏幕高-1.0f) continue;

            float dx_center = predictScreen.x - px;
            float dy_center = predictScreen.y - py;
            if (dx_center*dx_center + dy_center*dy_center > aimbot_range*aimbot_range) continue;

            float dx = predictScreen.x - px;
            float dy = predictScreen.y - py;
        ...
        if (touch_status==1&&aimbot_status==0||touch_status==2&&aimbot_status==1||touch_status==1&&aimbot_status==2) {
