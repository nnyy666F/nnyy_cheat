...
void game_wwqy::update() {
    ...
    for (int i=0;i<eneity_count;i++) {
        ...
        if (ReadFloat(ReadValue(object + 0x198) + 0x134)!=0&&ReadFloat(ReadValue(object + 0x198) + 0x134+8)!=-90000){
            ...
            driver->read(ReadValue(object + 0x1218) + 0x1f0, &pos[i], sizeof(this->pos[i]));
            isMiss[i] = true;
        }                              
    ...
    while(true){
        ...
        for (int i = 0; i < eneity_count; i++) {
            ...
            if (has_last_world[i]) {
                ...
                float current_fps = std::clamp(static_cast<float>(imgui_fps), 30.0f, 120.0f);
