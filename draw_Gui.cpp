...
void drawBegin()
{
	if (::permeate_record_ini)
	{
		...
		::window =
			android::ANativeWindowCreator::Create("test", native_window_screen_x,
												  native_window_screen_y, permeate_record);
		...
		permeate_record_ini = false;	// 修复：原代码此处未重置，可能导致重复执行
	}
...
void *read_data_thread(void *arg)
{
	...
	while (!stop_update_thread.load(std::memory_order_acquire))
	{
		...
		game.update();
		pthread_mutex_unlock(&game_data_mutex);
		usleep((100 * 10000) / game.imgui_fps);	// 60帧
	}
	printf("更新线程已停止\n");
	return nullptr;
}
